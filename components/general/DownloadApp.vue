    ...
    <div id="download_app_section" class="download_app_section_content_wrapper">
      <div class="container-xl">
        <div class="row align-items-center justify-content-center">
          <!-- Start:: Section Text -->
          <div class="col-lg-7 col-12" data-aos-once="true" :data-aos="$i18n.locale == 'ar' ? 'fade-left' : 'fade-right'"
            ...
            <div class="section_text_wrapper">
              ...
              <p class="section_desc text-center" v-html="$t('downloadText')"></p>
...
export default {
  ...
  methods: {
    async getData() {
      try {
        return await this.$axios.get(`settings?key=landing_page`).then(response => {
          this.isLoading = true;
          this.appStore = response.data.data[0].value.appStore;
          this.googlePlay = response.data.data[0].value.googlePlay;
        }).catch(error => {
          console.log(error)
        })
      } catch (error) {
